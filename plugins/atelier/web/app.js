...
/*
 * PORTED VERBATIM from agent-gw, with four URLs rewritten and nothing else.
 *
 ...
 * match the rest of Golem would have meant re-deriving every one of those
 ...
 * (see below), because agent-gw bundled this file with its launcher and Golem
 * does not.
 */
export default function createAtelierApp(api) {
  ...
  /* The one line this port needed.
     ...
     lines below reference a variable this file never declares. Golem loads
     ...
     Bound as a plain const, which the launcher explicitly could not do (its
     `api.page` was a getter, null until a view was routed). Here the adapter
     builds this app INSIDE the effect that owns the mounted element, so the
     node already exists — the hazard that rule guarded against is gone. */
  const page = api.page;
  /* Same story, same fix: `$` and `IC` were the launcher's too.

     `$` was `document.getElementById`. Scoped here to the page element
     instead — every id it looks up (`wbbody`, `shopmode`, `wbb*`) is one this
     engine wrote itself, so the behaviour is identical, and a plugin that
     cannot reach out of its own subtree is one that cannot collide with the
     shell by accident. */
  ...
  pieceModal.className = 'modal atelier-modal'; pieceModal.hidden = true;
  ...
  async function renderWorkbook(path) {
    ...
    try {
      const [rd, rs, rl] = await Promise.all([
        fetch('/api/plugin/atelier/workbook?path=' + encodeURIComponent(path), { headers: headers(false), cache: 'no-store' }),
        fetch('/api/plugin/atelier/state?wb=' + encodeURIComponent(path), { headers: headers(false), cache: 'no-store' }),
        fetch('/api/plugin/atelier/layout?wb=' + encodeURIComponent(path), { headers: headers(false), cache: 'no-store' }),
      ]);
  ...
  async function tick(id, done) {   // id = étape ; POST direct au serveur, jamais le LLM
    try { const r = await fetch('/api/plugin/atelier/state', { method: 'POST', headers: headers(true), body: JSON.stringify({ wb: wb.path, key: id, done }) }); if (r.ok) wb.state = await r.json(); } catch {}
    renderWb();
  ...
  async function wbSavePose(body) {
    try {
      const r = await fetch('/api/plugin/atelier/layout', { method: 'POST', headers: headers(true), body: JSON.stringify({ wb: wb.path, ...body }) });
