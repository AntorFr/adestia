/**
 * Nestor — the house's night-light, ported from the predecessor's livery.
 *
 * Neither the butler's drawing room nor the HUD: an OBJECT. The Nabaztag back
 * from the future — white porcelain, ears on stepper motors, and a belly that
 * lights up when it works, exactly what the object knew how to say on the
 * dresser. The audience is the household, not a technical adult: the landing
 * shows no gauge, no version, no tool trace. A rabbit and an invitation.
 *
 * NO `console` slot, deliberately. A status band that could only assert
 * "house serene" without measuring anything would be décor that lies; the
 * rabbit's belly already says "I am working", at the right moment.
 *
 * The paths are the canonical pose (ears up), traced from the "Nestor" face
 * the predecessor carries: `CORPS` is the full silhouette, `VISAGE` its HOLES
 * (eyes, muzzle) as reversed sub-paths, `CLOCHE` the bare skull that bounds
 * the ear mask. Carried VERBATIM — redrawing a mascot is how mascots die.
 */
...
/* Shared definitions, placed ONCE per document. The rabbit can appear twice
   on screen (the landing and the thread): embedding the <defs> in each
   instance would duplicate ids, and a duplicated mask applies to nobody. */
function ensureDefs() {
...
/**
 * The living rabbit. `taille` in pixels; `opts.affaire` races the belly,
 * `opts.surBulle` tells it the ground behind it is a bubble's, not the
 * page's — the face being HOLES, it must take that colour.
 */
function lapin(taille, opts) {
  ...
  el.className =
    'nst-lapin' + (o.affaire ? ' nst-affaire' : '') + (o.surBulle ? ' nst-sur-bulle' : '')
...
function home(host, context) {
  ...
  host.innerHTML = `<div class="nst-accueil">
