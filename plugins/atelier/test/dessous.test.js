...
/* ── Un fond se cote bout par bout, et les deux axes obéissent pareil ────────
   Il n'y a pas un « jeu du haut » et un « jeu du bas » : c'est le même, il
   rattrape la même erreur de coupe. Ce qui change, c'est ce qui TIENT chaque
   bout — une rainure, une pièce contre laquelle il bute, ou rien.

     rainure   il regagne (profondeur usinée − fond_jeu)
     bute      rien : le bout touche une pièce, c'est une référence
     libre     on retire marge_fond, pour qu'un panneau coupé large ne dépasse pas

   La largeur n'est qu'un cas particulier de la même règle — rainure des deux
   côtés — et un meuble rainuré en haut ET en bas se cotera tout seul le jour
   où il existera, sans une ligne de plus. */

test('le fond traversant flotte aux deux bouts : deux fois le même jeu', () => {
  const f = derive(garage(), tables()).pieces.find((p) => p.role === 'FOND')
  // 1919 de meuble, moins marge_fond deux fois.
  assert.equal(f.longueur, 1915)
})

test('tenu en bas, il ne se ménage qu\'en haut', () => {
  ...
  d.parametres = { ...d.parametres, rainure_bas_prof: 8 }
