...
`sessions`/`turns` also store cwd, repository (`AntorFr/adestia` was auto-detected from the git remote), branch, and full user/assistant message text — relevant to privacy expectations.
...
## 8. Driver-contract implications (Adestia `copilot-cli` driver)
