/**
 * The conversations a person keeps: listing, naming, reading, archiving.
 */
...
export function registerConversations(
  ...
  app.post<{ Body?: { title?: unknown } }>('/api/conversations', async (request) => {
    // The title comes with the creation rather than in a second call: a conversation
    // that exists for one round trip under the name "New conversation" is a
    // conversation that keeps that name whenever the second call is lost.
  ...
  app.post<{ Params: { id: string }; Body?: { archived?: unknown } }>(
    ...
    async (request, reply) => {
      // Reversible by construction: the same route brings a conversation back, so
