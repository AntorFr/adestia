...
| **Posture** | `open` / `ask` ; Adestia ne juge rien, elle relaie la question du moteur (26/08 v2) ; la couche de permissions a été retirée (26/08 v1), mais la POSTURE reste, et c'est le bloc `permissions:` qui la porte. | `permissions.mode` (config), `AskDesk`/`PendingAsk` (guichet), `permission-request` (événement), `interactivePermissions` (capacité), `/api/permission` (route), `scrubAsk`. | **Trois familles de mots (permission, ask, posture) pour un concept.** Rien à retirer — `permissions.mode` est porteur : il construit le guichet au démarrage et fait refuser le boot sur un moteur incapable de demander. C'est le MOT qui cloche, pas le bloc : la conception dit « posture », la config dit « permissions ». Même famille que le fil et la session — petit chantier, tenu par le compilateur. |
