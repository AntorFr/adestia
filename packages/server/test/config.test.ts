...
describe('environment overrides', () => {
  it('overrides only deployment-specific values', () => {
    ...
    const config = parseConfig('auth:\n  mode: proxy\n', {
      ADESTIA_HOST: '0.0.0.0',
      ADESTIA_PORT: '9000',
      ADESTIA_DATA_DIR: '/data',
      ADESTIA_WORKSPACE: '/workspace',
    })
...
`)
  ...
  it('was accepted and IGNORED before this existed', () => {
    ...
    expect(issuesOf('mcp:\n  serveurs: []\n')).toEqual([
      'mcp.serveurs is not a setting — known keys: servers, enabled, token, agentName, description, maxPending, ttlMs',
    ])
  })
...
`)
  ...
  it('refuses a placeholder the environment never filled', () => {
    ...
    expect(
      issuesOf(
        'mcp:\n  servers:\n    - name: m\n      url: https://h/m\n      auth:\n        tokenUrl: https://a/t\n        clientId: x\n        clientSecret: "${ADESTIA_HUB_SECRET}"\n',
      ),
    ).toEqual(['mcp.servers[0].auth.clientSecret is still "${ADESTIA_HUB_SECRET}" — that variable is not set'])
  })
...
describe('the instance name', () => {
  ...
  it('has no environment override, unlike where the instance runs', () => {
    ...
    expect(parseConfig('', { ADESTIA_NAME: 'Atelier' }).name).toBeUndefined()
  })
})
