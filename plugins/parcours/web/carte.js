...
//
// PORTÉ DU PRÉDÉCESSEUR, comme le moteur des voyages l'a été : mille lignes de
// rendu cartographique débogué (la projection, la mosaïque, les collisions de
// pastilles, le profil par distance cumulée, le mode balade), qu'une réécriture
// en React aurait obligé à re-mériter comportement par comportement.
//
// CE QUI A CHANGÉ, ET RIEN D'AUTRE : les coutures avec l'hôte.
//
// - `mountParcours` peint UN hôte qu'on lui donne, au lieu de balayer le
//   document à la recherche de `.parcours[data-src]`. C'est React qui monte le
//   bloc ici, il sait exactement quel élément et avec quel fichier ; le balayage
//   et le verrou `pc-loading` qui allait avec n'ont plus d'objet.
// - les deux URL de l'hôte (le GPX, la page d'un parcours) sont INJECTÉES.
//   Un plugin qui écrirait son propre préfixe en dur ne pourrait plus être
//   renommé — même raison que dans la vue des voyages.
// - le chemin mémoire n'est plus reconstitué en épluchant l'URL : l'appelant le
//   connaît, il le passe.
//
// CE QUI EST RESTÉ LÀ-BAS : « ⤓ Emporter », le hors-ligne. Il ne dépend pas de
// ce fichier mais d'un service worker qui sert le cache d'abord, et Adestia n'en
...
// `tuilesDuCorridor`) et revient le jour où Adestia devient une PWA.
//
...
function peindre(hote, data, relPath, urls) {
...
function peindreLien(hote, data, relPath, urls) {
  const trace = data.trace || {};
  hote.textContent = '';
  hote.classList.add('pc-ready', 'pc-asLink');

  const a = el('a', 'pc-card');
  a.href = urls.page(relPath);
...
/**
 * Peint un parcours dans l'élément qu'on lui donne.
 *
 * @param hote     l'élément à remplir. Il est vidé, puis peint.
 * @param src      l'URL où lire le `.parcours.json`.
 * @param chemin   le chemin du fichier dans l'espace de travail — ce que le
 *                 GPX et la page d'un parcours attendent, et que l'URL seule
 *                 ne redonne pas sans être épluchée.
 * @param vue      `carte` (défaut) ou `lien`.
 * @param urls     `{ gpx(chemin), page(chemin) }`. Un `gpx` absent retire le
 *                 bouton plutôt que de proposer un téléchargement qui échoue.
 * @returns de quoi défaire le montage — à appeler au démontage du bloc, sinon
 *          l'écouteur de redimensionnement survit à la page qui l'a posé.
 */
export function mountParcours(hote, { src, chemin, vue, urls = {} }) {
  let vivant = true;
  const peintre = vue === 'lien' ? peindreLien : peindre;
  ...
  fetch(src, { headers: { Accept: 'application/json' } })
    ...
    .then((data) => {
      // Le bloc a pu être démonté pendant la requête — on ne peint pas dans un
      // élément que React a déjà retiré de la page.
      if (vivant) peintre(hote, data, chemin, urls);
    })
    .catch((e) => {
      if (!vivant) return;
      hote.textContent = '';
