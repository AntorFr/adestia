...
name: voyage-json
description: >
  Le CONTRAT DE DONNÉES de l'app Voyages — le format de `<voyage>/assets/voyage.json`,
  que le front rend en timeline par jour et en tray de suggestions (glisser-déposer). À
  consulter dès que tu crées ou modifies un voyage : types d'item fermés, statuts,
  invariant de calage, traçabilité. Livré par le plugin avec le moteur qui le lit. Le
  métier — comment cadrer un voyage, quoi proposer — reste dans ton workspace.
---

# `voyage.json` — le contrat de données

Un dossier par voyage ; `assets/voyage.json` est la **source unique**. Le front le rend
...
- **`status` à la RACINE** — l'état du voyage, à ne pas confondre avec le `statut` d'un
  ...
  des voyages et dans l'en-tête. **`idée` ⇔ ni `debut` ni `fin`** : sans dates, la page
...
- **`ordre` et `heure`** : `ordre` est le rang de la carte dans son jour, posé par la
  ...
  optionnelle, jamais une grille. `creneau` (`matin`/`midi`/`apres-midi`/`soir`) survit
  comme simple conseil sur une suggestion, et sert de repli de tri. `duree` est du texte
  libre.
- **Traçabilité, zéro duplication** : `gmail` (fil source d'une résa — la vérité reste
  dans la boîte), `place_id` (la vérité du lieu reste chez maps). Un item `trajet`
  n'existe **que** porté par une résa ; un trajet routier estimé est une **liaison**,
  dérivée au rendu, jamais un item.
...
- **`hint`** : l'accroche **courte**, affichée sous le titre sur la carte du tray. Distincte
  de `desc`, la fiche rédigée que rend la modale — à défaut de `desc`, la modale se
  rabat sur `hint`. Une ligne : **rien ne la tronque**, une accroche trop longue fait
  grandir la carte.
- **`fiche`** : le seul lien **INTERNE** d'une carte — chemin **relatif au dossier du
  voyage** vers la page `.md` qu'elle ouvre (`citadelle-calvi.md`). La carte porte alors
...
- **`ico`** : un emoji, qui remplace le glyphe du type partout où la carte se rend. Le
  ...
  et s'afficherait en aviron. Optionnel ; échappé au rendu.
- **`docs`** : `[{ "fichier": "assets/…", "titre": "…" }]` — les pièces jointes des mails
  de résa, classées dans le dossier du voyage. Le fil reste la *source* ; le document est
...
La météo et les liaisons sont **dérivées** d'une clé Google que l'instance déclare
(`GOOGLE_MAPS_API_KEY`). Sans elle, l'app tourne : la timeline, le tray, les gestes,
tout vit — seuls les pictos météo et les chips de durée restent muets. **Ne compense
pas** en écrivant une durée dans `duree` : ce champ est pour ce qu'une résa affirme
(« ferry : 5 h 45 »), pas pour une estimation que la machine sait faire.
...
consolides ensuite, sur demande. Même frontière que les workbooks et la todo : **le
front ne touche jamais la mémoire.**

Concrètement, consolider = lire l'overlay, reporter `statut`/`jour`/`ordre`/`heure` sur
les items de `voyage.json`, puis **vider** `voyage-state.json` (`{"items": {}}`). Tant
que tu ne l'as pas fait, l'overlay gagne à l'affichage — donc rien ne se perd si tu
attends.
