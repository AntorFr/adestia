...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), 'adestia-start-'))
  logs = []
})
...
describe('what the instance installs as', () => {
  const writeSkin = async (id: string, fragment: unknown) => {
    ...
    await writeFile(
      join(root, 'skins', id, 'adestia-skin.json'),
  ...
  it("keeps the product's own manifest when no skin is worn", async () => {
    ...
    expect(manifest['name']).toBe('Adestia')
  ...
  it("still installs when the skin's fragment is unreadable", async () => {
    ...
    await writeFile(
      join(root, 'skins', 'amber', 'adestia-skin.json'),
      JSON.stringify({
        schemaVersion: 1,
        id: 'amber',
        description: 'A body.',
...
describe('the permission posture', () => {
  ...
  it('tolerates a permissions block written for an older Adestia', async () => {
