...
if (args.includes('--help') || args.includes('-h')) {
  console.log(`golem — a chat and apps interface on top of a coding-agent CLI

Usage: golem [options]

Options:
  -c, --config <path>   configuration file (default: golem.config.yaml)
  -h, --help            show this message

With no configuration file, Golem runs a single-user instance on
http://127.0.0.1:8730 with no authentication. See golem.config.example.yaml.`)
  process.exit(0)
}
...
if (flagIndex !== -1 && !configPath) {
  console.error('golem: --config needs a path')
  process.exit(2)
}
