...
export function Attachments({
  ...
  return (
    <section className="adestia-page-files">
      <h2 className="adestia-page-files__title">{t('Attached files')}</h2>

      {images.length > 0 && (
        <div className="adestia-page-files__grid">
          {images.map((file) => (
            ...
            <a
              key={file.path}
              className="adestia-page-files__thumb"
      ...
      {others.length > 0 && (
        <ul className="adestia-page-files__list">
          {others.map((file) => (
            <li key={file.path}>
              <a
                className="adestia-page-files__file"
                href={fileUrl(file.path)}
                target="_blank"
                rel="noreferrer"
              >
                <span className="adestia-page-files__glyph" aria-hidden="true">
                  {GLYPHS[file.kind]}
                </span>
                <span className="adestia-page-files__name">{file.name}</span>
                <span className="adestia-page-files__size">{humanSize(file.bytes, locale)}</span>
              </a>
            </li>
          ))}
        </ul>
